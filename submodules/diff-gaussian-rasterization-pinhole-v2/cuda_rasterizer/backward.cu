	...
	float* dL_dopacity)
{
	...
	float3 m_hom = transformPoint3x4(m, proj);
	float m_w = 1.0f / (m_hom.z + 0.0000001f);
	...
	glm::vec3 dL_dmean;	
	float mul1 = (proj[0] * m.x + proj[3] * m.y + proj[6] * m.z + proj[9]) * m_w * m_w;
	float mul2 = (proj[1] * m.x + proj[4] * m.y + proj[7] * m.z + proj[10]) * m_w * m_w;
	dL_dmean.x = (proj[0] * m_w - proj[2] * mul1) * dL_dmean2D[idx].x + (proj[1] * m_w - proj[2] * mul2) * dL_dmean2D[idx].y;
	dL_dmean.y = (proj[3] * m_w - proj[5] * mul1) * dL_dmean2D[idx].x + (proj[4] * m_w - proj[5] * mul2) * dL_dmean2D[idx].y;
	dL_dmean.z = (proj[6] * m_w - proj[8] * mul1) * dL_dmean2D[idx].x + (proj[7] * m_w - proj[8] * mul2) * dL_dmean2D[idx].y;
